...
run(function()
    local insta = {Enabled = false}
    insta = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({
        Name = "weird custom fly",
        Function = function(callback)
            if callback then
                -- Start flying the player
                local player = game.Players.LocalPlayer
                local character = player.Character or player.CharacterAdded:Wait()
                local humanoid = character:FindFirstChildOfClass("Humanoid")
                
                if humanoid then
                    humanoid.PlatformStand = true -- Prevents the character from falling over
                    local flySpeed = 50
                    ...
                    -- Handle disabling the module
                    insta.Function = function(callback)
                        if not callback then
                            flyConnection:Disconnect()
                            bodyVelocity:Destroy()
                            humanoid.PlatformStand = false
                ...
                -- Ensure to stop flying if the module is disabled
                local player = game.Players.LocalPlayer
                local character = player.Character or player.CharacterAdded:Wait()
                local humanoid = character:FindFirstChildOfClass("Humanoid")
